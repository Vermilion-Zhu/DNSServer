...
├── dns_cache.py                 # 🟢 缓存层：SQLite DNS 缓存
...
请求进入
  ...
  ├─ 3. SQLite 缓存查询 → 命中则直接返回
  ├─ 4. CNAME 缓存查询 → 命中则递归解析返回
  ├─ 5. 上游 DNS 转发 → 成功则缓存写入 + 返回
  └─ 6. 转发失败 → 返回 SERVFAIL
```
...
| `add_records(reply, qname)` | 将响应记录写入缓存 |
...
- 统一标签体系：`CACHE_HIT`、`CACHE_SET`、`DGA_BLOCKED`、`DGA_PASS`、`WHITELIST`、`UPSTREAM_ERR`、`FORWARD_ERR`、`CNAME_CHAIN` 等
...
| 方法 | 功能 |
|------|------|
| `get(domain, qtype)` | 查询缓存，返回 `(rdata, remaining_ttl)` 或 `None` |
| `set(domain, qtype, rdata, ttl)` | 写入缓存，自动计算过期时间 |
| `delete(domain, qtype)` | 删除指定记录 |
| `clear_expired()` | 清理所有过期记录 |
| `close()` | 关闭数据库连接 |
...
| 标签 | 含义 | 使用场景 |
|------|------|----------|
| `CACHE_HIT` | 缓存命中 | 服务器缓存查询命中 |
| `CACHE_SET` | 缓存写入 | 服务器写入缓存记录 |
| `CACHE_SET_CNAME` | CNAME 缓存写入 | 服务器写入 CNAME 缓存 |
...
| 单域名查询 | 输入域名，选择记录类型（A/AAAA/CNAME），通过本地服务器查询 |
| JSON 批量检测 | 从 JSON 文件加载域名列表批量检测，支持进度反馈与取消 |
...
1. **AI 增强安全**：将 RandomForest + Markov 链特征模型集成到 DNS 解析流程，实现实时 DGA 检测与拦截
2. **分层缓存加速**：SQLite 持久化缓存 + TTL 过期 + 后台自动清理，缓存命中可将响应时间从 ~0.2s 降至 <0.01s
3. **配置集中化**：`config.py` 统一管理所有参数，避免跨模块重复定义；支持 CLI 参数覆盖
4. **统一日志体系**：`logger.py` 提供多 Handler 架构（文件/控制台/GUI 控件），统一时间戳、标签和级别格式
5. **多工具协同**：CLI 客户端 + GUI 工具 + 服务器三位一体，覆盖不同使用场景
6. **向后兼容**：模型加载支持新旧格式自动识别，特征维度自动对齐
7. **线程安全**：SQLite 操作加锁，后台清理线程 daemon 模式，优雅退出机制
8. **CNAME 链解析**：递归解析 CNAME 链（最大深度 5），支持缓存加速中间步骤
9. **白名单机制**：精确匹配 + 后缀匹配双重机制，减少误报和性能开销
10. **GUI 日志捕获**：GUI 实时读取服务器子进程 stdout 输出，统一显示在日志区
11. **GUI 自动启动**：GUI 启动时自动启动本地 DNS 服务器子进程，无需手动操作
12. **上游 DNS 可配**：GUI 内可修改上游 DNS 服务器地址，重启服务器即时生效
13. **批量进度反馈**：批量检测时显示进度条和进度标签，支持随时取消任务
14. **子进程解耦**：GUI 与服务器通过子进程 + DNS 协议通信，服务器端功能扩展无需修改 GUI
